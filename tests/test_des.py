...
class TestDES(object):
    ...
    def test_des_invalid_key_size(self):
        with pytest.raises(ValueError) as exc:
            DES(b"\x01\x02")
        assert str(exc.value) == \
            "DES encryption key should be 8 bytes in length"

    def test_expand_56bit_key(self):
    ...
    def test_encrypt_large_bytes_padding(self):
        des = DES(b"PASSWORD")
        expected = b"\x15\x14\x4f\x75\x8c\x83\xd0\x34" \
